...
int init_outgsm_stru(const char* src_string, GSM_RECORD *cdr_stru)
{
	...
	/*国际出访把长途费放到基本费*/
	if(cdr_stru->roam_type[0]==ROAM_TYPE_OUTDM||cdr_stru->roam_type[0]==ROAM_TYPE_OUTHK)
	{
		(cdr_stru->cfee).old_fee += (cdr_stru->lfee).old_fee;
		(cdr_stru->cfee).fav_fee = (cdr_stru->cfee).old_fee;
		(cdr_stru->lfee).old_fee = 0;
		(cdr_stru->lfee).fav_fee = 0;
	}
	...
	strncpy((cdr_stru->cfee).product_id, "00000000000", 8);
	return 0;
}

/********************************************************** 
Function:		int init_szx_stru(const char* src_string, GSM_RECORD *cdr_stru)
Description:	初始化出访szx话单字符串到原始gsm话单结构
Input:			const char* src_string, 已经分解完成的话单字符串(含'\n')
Output:			GSM_RECORD *cdr_stru, 初始化完成的话单结构
Return: 		int 0 成功, -1表示出错
Others:			
**********************************************************/
int init_szx_stru(const char* src_string, GSM_RECORD *cdr_stru)
{
	int i = 0;
	char tmp[64], rtype;
	GSM_PREDEAL_RECORD *pre = cdr_stru->pre_record;
	...
	rtype = cdr_stru->roam_type[0];
	if(rtype == '1')
		cdr_stru->roam_type[0] == ROAM_TYPE_LOCAL;
	else if(rtype == '2')
		cdr_stru->roam_type[0] == ROAM_TYPE_OUTPROV;
	else
		cdr_stru->roam_type[0] == ROAM_TYPE_NONE;

	if(pre->call_type[0] == '0')
		strcpy(pre->call_type, NORMAL_CALL_TYPE_01);
	else
		strcpy(pre->call_type, NORMAL_CALL_TYPE_02);
...
int init_outforw_stru(const char* src_string, FORW_RECORD *cdr_stru)
{
	...
	strncpy((pri->cfee).product_id, "00000000000", 8);
	return 0;
}
...
int init_mont_stru(const char* src_string, MONT_RECORD *cdr_stru)
{
	...
	strncpy((cdr_stru->info_fee).product_id, "00000000000", 8);
	return 0;
}
...
int init_mms_stru(const char* src_string, MMS_RECORD *cdr_stru)
{
	...
	strncpy((cdr_stru->basic_fee).product_id, "00000000000", 8);
	return 0;
}
...
int init_ip_direct_stru(const char* src_string, IP_DIRECT_RECORD *cdr_stru)
{
	...
	memcpy(cf->deal_date, src_string+i,12);
	i+=12;
	memcpy(cdr_stru->lfee.discount_type, src_string+i,5 );
	i+=5 ;
	memcpy(cdr_stru->lfee.discount_order, src_string+i,5 );
	i+=5 ;
	memcpy(cdr_stru->lfee.present_info, src_string+i,7 );
	i+=7 ;
	memcpy(cdr_stru->lfee.sum_present_info, src_string+i,9 );
	i+=32;
	memcpy(cdr_stru->resv, src_string+i,15);
	strncpy(cdr_stru->lfee.group_id, "00000000000000000", 15);
	strncpy(cdr_stru->lfee.product_id, "00000000000", 8);
...
int init_phone_meeting_stru(const char* src_string, PHMT_RECORD *cdr_stru)
{
	...
	strncpy((cdr_stru->function_fee).product_id, "00000000000", 8);
...
int init_cp_stru(const char* src_string, CP_RECORD *cdr_stru)
{
	...
	i+=9;
	memcpy(cdr_stru->company_code, src_string+i, 20);
	i+=20;
	memcpy(cf->home_area_code,src_string+i,4);
	trim_right(cf->home_area_code);
	i+=4;
	...
	strncpy(cdr_stru->product_id, "00000000000", 8);
...
int init_cl_stru(const char* src_string, CL_RECORD *cdr_stru)
{
	...
	strncpy(fee->product_id, "00000000000", 8);
...
int init_evn_stru(const char* src_string, EVN_RECORD *cdr_stru)
{
	...
	i+=10;
	memcpy(cdr_stru->calling_account_num, src_string+i,20);
	i+=20;
	memcpy(cdr_stru->calling_user_name, src_string+i,20);
	i+=20;
	memcpy(cdr_stru->calling_user_id, src_string+i,20);
	i+=20;
	memcpy(cdr_stru->calling_broad_number, src_string+i,16);
	i+=16;
	memcpy(cdr_stru->acct_item, src_string+i,4);
	i+=4;
	memcpy(cdr_stru->site_account_number, src_string+i,20);
	i+=20;
	memcpy(cdr_stru->site_user_name, src_string+i,20);
	i+=20;
	memcpy(cdr_stru->site_user_id, src_string+i,20);
	i+=20;
	memcpy(cdr_stru->site_id, src_string+i,16);
	i+=16;
	memcpy(cdr_stru->site_rate, src_string+i,2);
	i+=2;
	memcpy(cdr_stru->site_port_type, src_string+i,1 );
	i+=1 ;
	memcpy(cdr_stru->conference_id, src_string+i,10);
	i+=10;
	memcpy(cdr_stru->conference_type, src_string+i,2);
	i+=2;
	memcpy(cdr_stru->service_starttime, src_string+i,14);
	i+=14;
	memcpy(cf->start_datetime, src_string+i,14);
	i+=14;
	memcpy(cdr_stru->end_time, src_string+i,14);
	i+=14;
	memcpy(cdr_stru->service_stoptime, src_string+i,14);
	i+=14;
	memcpy(cdr_stru->book_stoptime, src_string+i,14);
	i+=14;
	memcpy(cdr_stru->conf_cancel_time, src_string+i,14);
	i+=14;
	memcpy(cdr_stru->real_use_time_length, src_string+i,10);
	i+=10;
	memcpy(cdr_stru->time_length, src_string+i,10);
	i+=10;
	memcpy(cdr_stru->time_base_rate, src_string+i,10);
	i+=10;
	memcpy(cdr_stru->time_charge_unit, src_string+i,10);
	i+=10;
	memcpy(tmp, src_string+i,9);
	tmp[9] = 0;
	(cdr_stru->fee).fav_fee = atoi(tmp);
	i+=9;
	memcpy(tmp, src_string+i,9);
	tmp[9] = 0;
	(cdr_stru->fee).old_fee = atoi(tmp);
	i+=9;
	memcpy((cdr_stru->fee).discount_type, src_string+i, 5);
	i+=5;
	memcpy((cdr_stru->fee).discount_order, src_string+i, 5);
	i+=5;
	memcpy(cdr_stru->file_name, src_string+i,20);
	i+=20;
	memcpy(cf->deal_date, src_string+i,12);
	i+=12;
	memcpy((cdr_stru->fee).present_info, src_string+i,7);
	i+=7;
	memcpy((cdr_stru->fee).sum_present_info, src_string+i,9);
	i+=32;
	memcpy(cdr_stru->resv, src_string+i,15);

	strncpy((cdr_stru->fee).group_id, "00000000000000000", 15);
	strncpy((cdr_stru->fee).product_id, "00000000000", 8);
...
/********************************************************** 
Function:int init_ussd_stru(const char* src_string, USSD_RECORD *cdr_stru)
Description:初始化USSD话单字符串到原始会议电话话单结构
Input:const char* src_string, 已经分解完成的话单字符串(不含'\n')
Output:USSD_RECORD *cdr_stru, 初始化完成的话单结构
Return:int 0 成功, -1表示出错
Others:			
**********************************************************/
int init_ussd_stru(const char* src_string, USSD_RECORD *cdr_stru)
{
	int i = 0;
	char tmp[16];
	COMM_FIELD *cf = &(cdr_stru->comm_field);

	memset(cdr_stru, 0, sizeof(USSD_RECORD));
	memcpy(cf->system_type, src_string+i,2 );
	i+=2 ;
	memcpy(cdr_stru->bill_id, src_string+i,16);
	i+=16;
	memcpy(cdr_stru->caller_type, src_string+i,2 );
	i+=2 ;
	memcpy(cf->user_type, src_string+i,1);
	i+=1 ;
	memcpy(cf->msisdn, src_string+i,15);
	trim_right(cf->msisdn);
	i+=15;
	memcpy(cdr_stru->serv_code, src_string+i,15);
	i+=15;
	memcpy(cdr_stru->serv_type, src_string+i,2);
	i+=2;
	memcpy(cdr_stru->ussdc_id, src_string+i,8);
	i+=8;
	memcpy(cdr_stru->bill_type, src_string+i,1);
	i+=1;
	memcpy(cf->start_datetime, src_string+i,14);
	i+=14;
	memcpy(cdr_stru->call_end_time, src_string+i,14);
	i+=14;
	memcpy(cdr_stru->call_info_size, src_string+i,6 );
	i+=6 ;
	memcpy(cdr_stru->call_time, src_string+i,6);
	i+=6 ;
	memcpy(tmp, src_string+i,7 );
	tmp[7]=0;
	(cdr_stru->call_fee).fav_fee = atoi(tmp);
	i+=7 ;
	memcpy(cdr_stru->callInteractiveCount, src_string+i,4);
	i+=4;
	memcpy(cdr_stru->callMSRequestCount, src_string+i,4);
	i+=4;
	memcpy(cdr_stru->callServiceRequestCount, src_string+i,4);
	i+=4;
	memcpy(cdr_stru->callServiceNotifyCount, src_string+i,4);
	i+=4;
	memcpy(cdr_stru->callReLEaseStatus, src_string+i,2);
	i+=2;
	memcpy(cdr_stru->chargeIndex, src_string+i,4);
	i+=4;
	memcpy(tmp, src_string+i,7 );
	tmp[7]=0;
	(cdr_stru->info_fee).fav_fee = atoi(tmp);
	i+=7;
	memcpy(cdr_stru->chargeType, src_string+i,2);
	i+=2;
	memcpy(cdr_stru->chargeSpCode, src_string+i,6);
	i+=6;
	memcpy(cdr_stru->chargeOperCode, src_string+i,15);
	i+=15;
	memcpy(cdr_stru->imsi, src_string+i,15);
	i+=15;
	memcpy(cdr_stru->imei, src_string+i,20);
	i+=20;
	memcpy(cf->home_area_code, src_string+i,4 );
	trim_right(cf->home_area_code);
	i+=4 ;
	memcpy(tmp, src_string+i,7 );
	tmp[7]=0;
	(cdr_stru->call_fee).old_fee = atoi(tmp);
	i+=7;
	memcpy(tmp, src_string+i,7 );
	tmp[7]=0;
	(cdr_stru->info_fee).old_fee = atoi(tmp);
	...
	memcpy(cf->deal_date, src_string+i,12);
	i+=12;
	memcpy((cdr_stru->call_fee).discount_type, src_string+i,5 );
	i+=5 ;
	memcpy((cdr_stru->call_fee).discount_order, src_string+i,5 );
	i+=5 ;
	memcpy((cdr_stru->call_fee).present_info, src_string+i,7);
	i+=7;
	memcpy((cdr_stru->call_fee).sum_present_info, src_string+i,9);
	i+=32;
	memcpy(cdr_stru->resv, src_string+i,15);

	strncpy((cdr_stru->call_fee).group_id, "00000000000000000", 15);
	strncpy((cdr_stru->call_fee).product_id, "00000000000", 8);

	return 0;
}

/********************************************************** 
Function:int init_wad_stru(const char* src_string, WAD_RECORD *cdr_stru)
Description:初始化WAD话单字符串到原始会议电话话单结构
Input:const char* src_string, 已经分解完成的话单字符串(不含'\n')
Output:WAD_RECORD *cdr_stru, 初始化完成的话单结构
Return:int 0 成功, -1表示出错
Others:			
**********************************************************/
int init_wad_stru(const char* src_string, WAD_RECORD *cdr_stru)
{
	int i = 0;
	char tmp[16];
	COMM_FIELD *cf = &(cdr_stru->comm_field);

	memset(cdr_stru, 0, sizeof(WAD_RECORD));
	memcpy(cf->system_type, src_string+i,2 );
	i+=2 ;
	memcpy(cf->msisdn, src_string+i,15);
	trim_right(cf->msisdn);
	i+=15;
	memcpy(cf->other_party, src_string+i,15);
	i+=15;
	memcpy(cf->start_datetime, src_string+i,14);
	i+=14;
	memcpy(cdr_stru->end_datetime, src_string+i,14);
	...
	memcpy(cdr_stru->duration, src_string+i,6 );
	i+=6 ;
	memcpy(cdr_stru->bill_flag, src_string+i,6);
	i+=6 ;
	memcpy(cf->home_area_code, src_string+i,4);
	trim_right(cf->home_area_code);
	i+=4;
	...
	switch(src_string[0])
	{
		case 'v': /*GSM*/
		{
			switch(src_string[1])
			{
				...
				case 's': /*视讯通*/
				{
					if(len == VS_RECORD_LEN && !init_evn_stru(src_string, (EVN_RECORD*)cdr_stru))
						flag = 0;
					break;
				}
				default:
		...
		case 'w': /*wlan*/
		{
			...
			if(len == WA_RECORD_LEN && !init_wad_stru(src_string, (WAD_RECORD*)cdr_stru))
				flag = 0;
		...
		case 'g': 
		{
			switch(src_string[1])/*gprs*/
			{
				...
				case 'n':/*上网直通车*/
				{
					if(len == GN_RECORD_LEN && !init_www_direct_stru(src_string, (WWW_DIRECT_RECORD*)cdr_stru))
						flag = 0;
		...
		{
			/*xuxg 20050604添加io*/			
			switch(src_string[1]) /*神州行*/
			{
				case 'p':
				{			
					if(len == IP_RECORD_LEN && !init_szx_stru(src_string, (GSM_RECORD*)cdr_stru))
						flag = 0;
					break;
				}
				case 'o': /*mont*/
		...
		case 'f': /*护身宝*/
		{
			if(len == FN_RECORD_LEN && !init_fns_stru(src_string, (FNS_RECORD*)cdr_stru))
				flag = 0;
			break;
		}
		case 't': /*会议电话*/
		{
			if(len == TM_RECORD_LEN && !init_mcall_stru(src_string, (MCALL_RECORD*)cdr_stru))
				flag = 0;
			break;
		}
		case 'u': /*ussd*/
		{
			if(len == USSD_RECORD_LEN && !init_ussd_stru(src_string, (USSD_RECORD*)cdr_stru))
				flag = 0;
			break;
		}
		default:
...
int format_out_gen_sms(char* src_string, GEN_SMS_RECORD *cdr_stru)
{
	...
	sprintf(src_string, "%-2.2s%-2.2s%-15.15s%-15.15s%-20.20s%-15.15s%-24.24s%-14.14s%-14.14s%-3.3s%-3.3s%-3.3s%-4.4s%-3.3s%-1.1s%07d%07d%-4.4s%07d%07d%-20.20s%-12.12s%-5.5s%-5.5s%-7.7s%-9.9s%-15.15s%-8.8s%-15.15s\n",
...
int format_out_ip_direct(char* src_string, IP_DIRECT_RECORD *cdr_stru)
{
	...
	sprintf(src_string, "%-2.2s%-2.2s%-2.2s%-15.15s%-20.20s%-15.15s%-15.15s%-24.24s%-14.14s%-6.6s%-11.11s%-10.10s%-3.3s%-3.3s%-4.4s%-4.4s%-4.4s%-4.4s%-4.4s%-1.1s%-1.1s%-1.1s%-7.7s%09d%-7.7s%09d%-20.20s%-12.12s%-5.5s%-5.5s%-7.7s%-9.9s%-15.15s%-8.8s%-15.15s\n",
	cf->system_type,          		
	cdr_stru->key_type,             
	cdr_stru->call_type,            
	cdr_stru->imsi_number,          
	...
	cf->deal_date,            
	cdr_stru->lfee.discount_type,        
	cdr_stru->lfee.discount_order,       
	cdr_stru->lfee.present_info,         
	cdr_stru->lfee.sum_present_info,     
 	cdr_stru->lfee.group_id,             
	cdr_stru->lfee.product_id,           
	cdr_stru->resv);                 
...
int format_out_phone_meeting(char* src_string, PHMT_RECORD *cdr_stru)
{
	...
	(cdr_stru->function_fee).product_id,           
	cdr_stru->resv);                 
...
int format_out_cp(char* src_string, CP_RECORD *cdr_stru)
{
	...
	sprintf(src_string, "%-2.2s%-15.15s%-14.14s%-9.9s%-20.20s%-4.4s%-20.20s%-12.12s%-5.5s%-15.15s%-8.8s%-15.15s\n",
	cf->system_type,
	cf->msisdn,
	cf->start_datetime,
	cdr_stru->fee,
...
int format_out_www_direct(char* src_string, WWW_DIRECT_RECORD *cdr_stru)
{
	...
	sprintf(src_string, "%-2.2s%-4.4s%-1.1s%-15.15s%-14.14s%-14.14s%-10.10s%-8.8s%-32.32s%-40.40s%-16.16s%-16.16s%-10.10s%-10.10s%-10.10s%-10.10s%-10.10s%-38.38s%-16.16s%-16.16s%-40.40s%-16.16s%-16.16s%-8.8s%-6.6s%-4.4s%-2.2s%-2.2s%-7.7s%-7.7s%-9.9s%-7.7s%-7.7s%-9.9s%-20.20s%-12.12s%-5.5s%-5.5s%-7.7s%-9.9s%-15.15s%-8.8s%-15.15s\n",
	cf->system_type,
	cdr_stru->sm_type,
	cdr_stru->billing_type,
	cf->msisdn,
...
int format_out_evn(char* src_string, EVN_RECORD *cdr_stru)
{
	...
	sprintf(src_string, "%-2.2s%-15.15s%-10.10s%-20.20s%-20.20s%-20.20s%-16.16s%-4.4s%-20.20s%-20.20s%-20.20s%-16.16s%-2.2s%-1.1s%-10.10s%-2.2s%-14.14s%-14.14s%-14.14s%-14.14s%-14.14s%-14.14s%-10.10s%-10.10s%-10.10s%-10.10s%09d%09d%-5.5s%-5.5s%-20.20s%-12.12s%-7.7s%-9.9s%-15.15s%-8.8s%-15.15s\n",
	cf->system_type,
	cf->msisdn,
	cdr_stru->serial_num,
	cdr_stru->calling_account_num,
	cdr_stru->calling_user_name,
	cdr_stru->calling_user_id,
	cdr_stru->calling_broad_number,
	cdr_stru->acct_item,
	cdr_stru->site_account_number,
	cdr_stru->site_user_name,
	cdr_stru->site_user_id,
	cdr_stru->site_id,
	cdr_stru->site_rate,
	cdr_stru->site_port_type,
	cdr_stru->conference_id,
	cdr_stru->conference_type,
	cdr_stru->service_starttime,
	cf->start_datetime,
	cdr_stru->end_time,
	cdr_stru->service_stoptime,
	cdr_stru->book_stoptime,
	cdr_stru->conf_cancel_time,
	cdr_stru->real_use_time_length,
	cdr_stru->time_length,
	cdr_stru->time_base_rate,
	cdr_stru->time_charge_unit,
	(cdr_stru->fee).fav_fee,
	(cdr_stru->fee).old_fee,
	(cdr_stru->fee).discount_type,
	(cdr_stru->fee).discount_order,
	cdr_stru->file_name,
	cf->deal_date,
	(cdr_stru->fee).present_info,
	(cdr_stru->fee).sum_present_info,
 	(cdr_stru->fee).group_id,
	(cdr_stru->fee).product_id,        
	cdr_stru->resv);
...
int format_out_fns(char* src_string, FNS_RECORD *cdr_stru)
{
	...
	sprintf(src_string, "%-2.2s%-2.2s%-15.15s%-20.20s%-15.15s%-24.24s%-14.14s%-14.14s%-6.6s%11.11s%-10.10s%-5.5s%-5.5s%-1.1s%-1.1s%-1.1s%-3.3s%-3.3s%-4.4s%-4.4s%-4.4s%-4.4s%-6.6s%-7.7s%-9.9s%-7.7s%-7.7s%-9.9s%-7.7s%-8.8s%-8.8s%-4.4s%-20.20s%-20.20s%-12.12s%-5.5s%-5.5s%-7.7s%-9.9s%-15.15s%-8.8s%-15.15s\n",
	cf->system_type,	        
	cdr_stru->call_type,	        
	cdr_stru->imsi_number,	        
	cdr_stru->imei_number,	        
...
int format_out_mcall(char* src_string, MCALL_RECORD *cdr_stru)
{
	...
	sprintf(src_string, "%-2.2s%-12.12s%-2.2s%-15.15s%-24.24s%-14.14s%-14.14s%-6.6s%-7.7s%-7.7s%-1.1s%-4.4s%-4.4s%-1.1s%-1.1s%-1.1s%09d%-9.9s%-9.9s%-9.9s%-9.9s%-9.9s%-20.20s%-20.20s%-20.20s%-20.20s%09d%-9.9s%-9.9s%-9.9s%-9.9s%-9.9s%-20.20s%-12.12s%-5.5s%-5.5s%-7.7s%-9.9s%-15.15s%-8.8s%-15.15s\n",
	cf->system_type,
	cdr_stru->card_code,
	cdr_stru->oper_code,
	cf->msisdn,
...
/********************************************************** 
Function:int format_out_ussd(char* src_string, USSD_RECORD *cdr_stru)
Description:将批价后ussd话单结构格式化输出到字符串中
Input:USSD_RECORD *cdr_stru, 批价后ussd话单结构
Output:const char* src_string, 已经格式化完成的话单字符串(含'\n')
Return:int 0 成功, -1表示出错
Others:			
**********************************************************/
int format_out_ussd(char* src_string, USSD_RECORD *cdr_stru)
{
	COMM_FIELD *cf = &(cdr_stru->comm_field);

	sprintf(src_string, "%-2.2s%-16.16s%-2.2s%-1.1s%-15.15s%-15.15s%-2.2s%-8.8s%-1.1s%-14.14s%-14.14s%-6.6s%-6.6s%07d%-4.4s%-4.4s%-4.4s%-4.4s%-2.2s%-4.4s%07d%-2.2s%-6.6s%-15.15s%-15.15s%-20.20s%-4.4s%07d%07d%-20.20s%-12.12s%-5.5s%-5.5s%-7.7s%-9.9s%-15.15s%-8.8s%-15.15s\n",
	cf->system_type,
	cdr_stru->bill_id,
	cdr_stru->caller_type,
	cf->user_type,
	cf->msisdn,
	cdr_stru->serv_code,
	cdr_stru->serv_type,
	...
	cdr_stru->bill_type,
	cf->start_datetime,
	cdr_stru->call_end_time,
	cdr_stru->call_info_size,
	cdr_stru->call_time,
	(cdr_stru->call_fee).fav_fee,
	cdr_stru->callInteractiveCount,
	cdr_stru->callMSRequestCount,
	cdr_stru->callServiceRequestCount,
	cdr_stru->callServiceNotifyCount,
	cdr_stru->callReLEaseStatus,
	cdr_stru->chargeIndex,
	(cdr_stru->info_fee).fav_fee,
	cdr_stru->chargeType,
	cdr_stru->chargeSpCode,
	cdr_stru->chargeOperCode,
	cdr_stru->imsi,
	cdr_stru->imei,
	cf->home_area_code,
	(cdr_stru->call_fee).old_fee,
	(cdr_stru->info_fee).old_fee,
	cdr_stru->file_name,
	cf->deal_date,
	(cdr_stru->call_fee).discount_type,
	(cdr_stru->call_fee).discount_order,
	(cdr_stru->call_fee).present_info,
	(cdr_stru->call_fee).sum_present_info,
 	(cdr_stru->call_fee).group_id,
	(cdr_stru->call_fee).product_id,
	cdr_stru->resv);
...
/********************************************************** 
Function:int format_out_wad(char* src_string, WAD_RECORD *cdr_stru)
Description:将批价后wad话单结构格式化输出到字符串中
Input:WAD_RECORD *cdr_stru, 批价后wad话单结构
Output:const char* src_string, 已经格式化完成的话单字符串(含'\n')
Return:int 0 成功, -1表示出错
Others:			
**********************************************************/
int format_out_wad(char* src_string, WAD_RECORD *cdr_stru)
{
	COMM_FIELD *cf = &(cdr_stru->comm_field);

	sprintf(src_string, "%-2.2s%-15.15s%-15.15s%-14.14s%-14.14s%-6.6s%-6.6s%-4.4s%07d%07d%-20.20s%-12.12s%-5.5s%-5.5s%-7.7s%-9.9s%-15.15s%-8.8s%-15.15s\n",
	cf->system_type,
	cf->msisdn,
	cf->other_party,
	cf->start_datetime,
