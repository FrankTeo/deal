...
/****************************************************************** 
Function:		int strupper(char *src_str)
Description:	将字符串中的字符转小写
Input:			char * src_str  需要转换的字符串
Return: 		int  0
Others:			
********************************************************************/
int strlower(char *src_str)
{
    while(*src_str != '\0')
    {
        *src_str = tolower(*src_str);
        src_str++;
    }
...
/*将BLOCK_BUF清0*/
int clean_block_buf(BLOCK_BUF* block_buf)
{
	if(block_buf->inFp != NULL)
		fclose(block_buf->inFp);
...
/****************************************************************** 
Function:		int block_read(char* out_buf, BLOCK_BUF* block_buf, const char* des_file, const int min_len)
Description:	行为与fgets类似，以\n为分割点，将文件行读出。
Input:			BLOCK_BUF* block_buf, 外建缓冲区
				const char* des_file, 读取的目标文件全路径
				const int min_len, 最小行长
Output:			char* out_buf, 文件行输出缓冲区
Return: 		int  读取的字节数, 0表示文件读取完成, -1 文件读取失败 -2 文件行长超过1023
Others:			此函数的限制行长为1023个字节（含\n）,因此out_buf的最小长度为1024。
********************************************************************/
int block_read(char* out_buf, BLOCK_BUF* block_buf, const char* des_file, const int min_len)
{
	int size = 0, lt;
	...
	/*文件指针为空，表示是第一次读取*/
	if(block_buf->inFp == 0 || block_buf->inFp ==NULL)
	{
		struct stat fStat;

		if(stat(des_file, &fStat) != 0)
			return -1;

		if(fStat.st_size < 1)
			return 0;

		block_buf->totalSize = fStat.st_size;

		if((block_buf->inFp = fopen(des_file, "r")) == NULL)
			return -1;

		lt = BUFFER_NUM*BEST_BUF;

		if(lt > fStat.st_size)
			lt = fStat.st_size;

		if(fread(block_buf->masterBuf, lt, 1, block_buf->inFp) != 1)
		{
			clean_block_buf(block_buf);
			return -1;
		}

		block_buf->bufSize = lt;
	}

	/*已经读到文件尾部或文件为空*/
	if(block_buf->totalRead >= block_buf->totalSize)
	{
		clean_block_buf(block_buf);
		out_buf[0] = 0;
		return 0;
	...
	else
	{
		...
		/*如果缓冲区内有效内容长度大于等于最小长度*/
		if((size = block_buf->bufSize - block_buf->bufPos)>=min_len)
		{
			lt = min_len;

			for(i=block_buf->bufPos+min_len-1; i<block_buf->bufSize; i++)
			{
				if(block_buf->masterBuf[i]=='\n')
				{
					memcpy(out_buf, block_buf->masterBuf+block_buf->bufPos, lt);
					block_buf->totalRead += lt;
					block_buf->bufPos += lt;
					out_buf[lt] = 0;
					return lt;
				}
		...
		if(size>0)
		{
			memcpy(out_buf, block_buf->masterBuf+block_buf->bufPos, size);
		}

		if(0 < (lsize = block_buf->totalSize - block_buf->totalRead - size))
		{
			lt = BUFFER_NUM*BEST_BUF;
			...
			if(fread(block_buf->masterBuf, lsize, 1, block_buf->inFp) != 1)
			{
				clean_block_buf(block_buf);
				return -1;
			}

			block_buf->totalRead += size;
			block_buf->bufSize = lsize;
			block_buf->bufPos = 0;
			...
			for(i = 0; i < lsize; i++)
			{
				...
				if(block_buf->masterBuf[i]=='\n')
				{
					memcpy(out_buf+size, block_buf->masterBuf, lt);
					out_buf[lt+size] = 0;
					block_buf->totalRead += lt;
					block_buf->bufPos += lt;
					return lt+size;
		...
		}/*END if(0 < (lsize = block_buf->totalSize - block_buf->totalRead - size))*/
		else
		{
			block_buf->totalRead += size;
			block_buf->bufPos += size;
			out_buf[size] = 0;
		}
	}
		...
		if( strcmp(dirent->d_name,".") != 0 && strcmp(dirent->d_name,"..") != 0 )
		{
			if(check!=NULL)
			{
				if(check(dirent->d_name))
				{
					...
					if(move_file(tmp_path, tmp_file))
						return -1;
				}
			}
			else
...
/****************************************************************** 
Function:		int match_regex_plus(const char* src_str, const regex_t* regex)
Description:	正则表达式的规则匹配函数
Input:			const char* src_str 需要检测的字符串
                const regex_t* regex    编译后正则表达式
Return: 		int  1表示匹配，0表示不匹配，-1表示正则表达式有错
Others:			
********************************************************************/
int match_regex_plus(const char* src_str, const regex_t* regex)
{
	if(regexec(regex, src_str, (size_t) 0, NULL, 0) != 0)
	{
		return 0;
	}
