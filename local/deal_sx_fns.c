...
* File Name  : deal_sx_fns.c
* Author     : zhumy
* Version    : 1.0.0
* Create date: 2004-12-27
* Description: 处理山西护身宝业务批价
* Others     : 
* History    :
********************************************************************/
...
int get_fn_conditions(CommConditions *, FNS_RECORD*);
int check_fn_fields(FNS_RECORD*, time_t, int *);

/********************************************************** 
Function:		int deal_fns(BillPlus* bill_plus, RuntimeInfo *runtime_info, FileCache* file_cache)
Description:	处理山西护身宝业务
...
Others:			
**********************************************************/
int deal_fns(BillPlus* bill_plus, RuntimeInfo *runtime_info, FileCache* file_cache)
{
	...
	/*初始化原始话单到结构体, 并检错*/
	if(init_cdr_stru(bill_plus->original_bill, &l_fns))
	{
	...
	/*对原始字段进行检错*/
	if(check_fn_fields(&l_fns, runtime_info->cur_time, &(bill_plus->error_number)))
		return 0;

	comm_field = &(l_fns.comm_field);
	...
	/*获取附加条件*/
	get_fn_conditions(&comm_cdn, &l_fns);

	sprintf(l_fns.group_id, "%-15.15s", base_info[0].group_id);
	sprintf(l_fns.product_id, "%-8.8s", base_info[0].product_id);
	sprintf(comm_field->deal_date, "%-12.12s", comm_cdn.sys_time+2);

	/*格式化输出*/
	format_out_fns(bill_plus->dealed_bill, &l_fns);

	return 0;
}

/****************************************************************** 
Function:		int get_fn_conditions(CommConditions *comm_cdn, FNS_RECORD* fns)
Description:	获取fns业务批价附加条件
Input:			FNS_RECORD* fns, fns出口话单结构
Output:			CommConditions *comm_cdn, 批价需要使用的公共信息
Return: 		int  0表示正确, 非0 表示错误
Others:			
********************************************************************/
int get_fn_conditions(CommConditions *comm_cdn, FNS_RECORD* fns)
{
	char tmp[64];
...
/********************************************************** 
Function:		int check_fn_fields(FNS_RECORD* fns, time_t file_time, int *error_no)
Description:	对上网直通车入口原始字段检错
Input:			FNS_RECORD* fns, fns入口话单结构
				time_t file_time, 文件开始处理时间
Output:			int *error_no, 错误代码
Return: 		int 0 没有错误, 1 有错
Others:			
**********************************************************/
int check_fn_fields(FNS_RECORD* fns, time_t file_time, int *error_no)
{
	COMM_FIELD *comm_field = &(fns->comm_field);
