...
int child_process(RuntimeInfo *runtime_info)
{
	...
	/*写运行日志*/
	if(write_runlog(runtime_info, "", 0))
		return 1;
	...
	{
		write_runlog(runtime_info, "Resume from break point error", 10);
		return 1;
	}
	...
	if(dbconnect(runtime_info->login_str))
	{
		write_runlog(runtime_info, "Connect to database error", 10);
		return 3;
	}

	sprintf(tmp, "%s", (runtime_info->parameter_info)->src_dir);
	...
	{
		write_runlog(runtime_info, "Can not catch signal!\n", 10);
		return 3;
	}
#ifdef DEBUG_MEMLEAK
g_memleak =0;
	...
	if(sigsetjmp(jmpbuf, 1))
	{
		...
		/*断开数据库连接*/
		disconnect_database();
		write_runlog(runtime_info, "User send signal to terminate the process", 4);
		return 4;
	}
	...
	while(1)
	{
		...
		{
			write_runlog(runtime_info, "check sys proc", 3);
			exit_status = 1;
		...
		while((dirent = readdir(dir_s))!=NULL)
		{
			...
			if(!(tag = check_filename((runtime_info->parameter_info)->program_name, dirent->d_name)))
			{
				...
				gettimeofday(&(stat_info.start_time),NULL);
				runtime_info->cur_time = stat_info.start_time.tv_sec;

				/*处理文件*/
				if((exit_status = deal_file(runtime_info, dirent->d_name, &file_cache, &stat_info)))
				{
					fclose(runtime_info->roll_fp);
					runtime_info->roll_fp = NULL;
					break;
				}
				printf("deal_out_file |%s| ......... ", dirent->d_name);

				/*移出口、入口文件*/
				if(deal_out_file(runtime_info, &stat_info, log_file))
				{
					exit_status = 3;
					break;
				}

				gettimeofday(&(stat_info.end_time));
				
				/*记录处理日志*/
				write_deal_log(log_file[0], &stat_info);
		...
		{
			if((runtime_info->parameter_info)->host_flag[0] == '0')
				printf("There is not file! Sleep 10 sec .............\n");
