...
int deal_forw(BillPlus* bill_plus, RuntimeInfo *runtime_info, FileCache* file_cache)
{
	...
	/*
	 if(memcmp(prcd->roam_type,ROAM_TYPE_INTER,1) && atoi((prcd->pre_record)->call_duration)<1)   
	 {    
	...
	/*获取费用类型*/
	if(get_fee_type(prcd->fee_type, prcd, bill_plus->inter_type, comm_cdn.sys_time, runtime_info->communal_data))
	{
	...
	/*求基本费*/
	if((flag = get_cfee(prcd, &comm_cdn, runtime_info, file_cache, MFLAG_FFEE)))
	{
	...
	/*求信息费*/
	if((flag = get_info_fee(prcd, &comm_cdn, bill_plus->dealed_other + bill_plus->other_pos, runtime_info, file_cache)))
	{
	...
	/*求长途费*/
	if((flag = get_lfee(prcd, &comm_cdn, runtime_info, file_cache)))
	{
