...
others: 
*/
int BaseFile_CreateFile(struct stBaseFile_Data *obj)
{
	int i;
	...
	{
		AUTOCALL_DEBUG;
		return BASEFILE_ERROR_CLOSEFILE;
	}
...
void BaseFile_Func_SprintRecord(struct stBaseFile_Record *obj, char *data)
{
	...
	memset(buffer, 0, sizeof(buffer));
	sprintf(buffer, "%08d", obj->free_value);
